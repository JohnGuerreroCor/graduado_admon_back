...
public class ConsultaGeneralDaoImpl implements IConsultaGeneralDao {
	...
	@Override
	public List<ProgramaGeneral> obtenerProgramasFacultad(int sedeCodigo, int nivelAcademicoCodigo,
			int dependenciaCodigo) {
		...
		String sql = "select p.pro_codigo, u.uaa_nombre, u.uaa_nombre_corto, u.uaa_dependencia, u2.uaa_nombre as facultad, p.sed_codigo, s.sed_nombre from programa p "
				...
				+ "inner join sede s on p.sed_codigo = s.sed_codigo " + "where s.sed_codigo = ? "
				+ " and p.nia_codigo = ? and u2.uaa_codigo = ? "
				+ " and p.pro_estado = 1 and p.pro_registro_snies is not null and p.pro_registro_snies != '' order by u.uaa_nombre asc";
		return jdbcTemplate.query(sql, new Object[] { sedeCodigo, nivelAcademicoCodigo, dependenciaCodigo },
				new ProgramaGeneralSetExtractor());
	...
	public List<NivelAcademico> obtenerNivelesAcademicos(int nivelFormacionCodigo) {
		...
		String sql = "select * from dbo.nivel_academico na where na.nia_estado = 1 and na.nat_codigo = ? order by na.nia_codigo desc";
		return jdbcTemplate.query(sql, new Object[] { nivelFormacionCodigo }, new NivelAcademicoSetExtractor());
