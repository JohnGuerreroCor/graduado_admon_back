...
public class InicioSesionDaoImpl implements IInicioSesionDao {
	...
	public String obtenerTokenInicioSesion(String atributos, String userdb) {
		...
		String token = null;
		try {
			token = jdbcTemplateConsulta.queryForObject("SELECT token.getTokenInicioSesion(?)",
					new Object[] { atributos }, String.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return token;
	...
	public String urlTokenPeticion(String userdb) {
		...
		String url = null;
		try {
			url = jdbcTemplateConsulta.queryForObject(
					"SELECT wep_valor FROM dbo.web_parametro wp where wp.wep_codigo = 377", String.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return url;
	...
	public String obtenerSegundaClaveReal(String segundaClave) {
		...
		String claveReal = jdbcTemplateLogin.queryForObject(sql, new Object[] { segundaClave },
